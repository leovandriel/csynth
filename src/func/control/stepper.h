...
static csError stepper_init(__U size_t count, __U Gen **args, void *context_)
{
    StepperContext *context = (StepperContext *)context_;
...
Func *stepper_create(int key, double value, double step, double min, double max, bool rel)
{
    ...
    return func_create(stepper_init, stepper_eval, control_event_free, NULL, sizeof(StepperContext), &initial, FuncFlagSkipReset);
}

...
static double karplus_strong_eval(__U int count, Gen **args, Eval eval, void *context_)
{
    ...
    double sustain_tick = gen_eval(args[1], eval);
    ...
    buffer[context->index] = 0.5 * (buffer[context->index] + buffer[next]) / exp2(sustain_tick);
...
Func *karplus_strong_osc(Func *pitch_tick, Func *decay_tick)
{

...
static double karplus_strong_eval(__U int count, Gen **args, Eval eval, void *context_)
{
    ...
    size_t size = (size_t)round(1.0 / gen_eval(args[0], eval));
    ...
    buffer[context->index] = 0.5 * (buffer[context->index] + buffer[next]) / exp2(gen_eval(args[1], eval));
...
Func *karplus_strong_gen(Func *pitch_ticker, Func *decay_tick)
{
    return func_create(NULL, karplus_strong_eval, karplus_strong_free, sizeof(KarplusStrongContext), NULL, FuncFlagNone, FUNCS(pitch_ticker, decay_tick));
}

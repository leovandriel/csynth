 ...
 *
 * Gaussian noise follows a normal distribution with mean 0 and standard
 * deviation 1, creating noise that clusters around the center value with
 * decreasing probability toward the extremes. This produces a more natural
 * sounding noise compared to uniform white noise, as it better matches many
 * real-world random processes.
 *
 * The output follows the standard normal distribution N(0,1), meaning
 * approximately:
 * - 68% of values fall within ±1.0
 * - 95% of values fall within ±2.0
 * - 99.7% of values fall within ±3.0
 *
 * Each call returns a new random value using a dedicated random number
 * generator initialized with seed 0. The values are independent of previous
 * outputs.
 *
 * Common uses include:
 ...
 *
 * @return Func* Gaussian noise function that outputs normally distributed
 * values.
 */

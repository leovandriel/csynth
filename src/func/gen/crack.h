 ...
 * (controlled by tick) of being a random value in [-1, 1], otherwise it outputs
 * 0.
 *
 ...
 *
 * The random values and timing use a dedicated random number generator
 * initialized with seed 0, so the pattern will be consistent between runs but
 * independent of other random generators.
 *
 * @param tick Function controlling probability of crackle. Should output values
 *            in range [0, 1]. Higher values create more frequent crackles: -
 *            0.0: Silent (no crackles) - 0.1: Sparse occasional crackles - 0.5:
 *            Moderate density - 1.0: Maximum density (random noise)
 * @return Func* Crackle function that outputs random values or zero based on
 * probability.
 */

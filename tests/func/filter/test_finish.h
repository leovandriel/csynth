...
{
    Func *input = add_create(ARGS(impulse_create(), delay_create(const_create(0.2), impulse_create())));
...
{
    Func *input = add_create(ARGS(impulse_create(), delay_create(const_create(0.1), impulse_create())));
    Func *trunc = finish_create(const_create(1.25), input);
    Gen *gen = gen_create(trunc);
    assert_double_equal(gen_eval(gen, NULL), 1.0);
    assert_double_equal(gen_eval(gen, NULL), 0.0);
